...
class KeyTest < ActiveSupport::TestCase
  ...
  test "create key object" do
    assert Key.new(@node, "hdm::integer")
  end

  test "two keys are equal when their environment, node and name match" do
    key_one = Key.new(@node, "hdm::integer")
    key_two = Key.new(@node, "hdm::integer")
  ...
  test "two keys are not equal if their environments differ" do
    ...
    hdm_node = Node.new(hostname: "testhost", environment: hdm_environment)
    key_one = Key.new(@node, "hdm::integer")
    key_two = Key.new(hdm_node, "hdm::integer")
  ...
  test "two keys are not equal if their nodes differ" do
    ...
    key_one = Key.new(@node, "hdm::integer")
    key_two = Key.new(other_node, "hdm::integer")
    assert_not_equal key_one, key_two
  end

  test "two keys are not equal if their names differ" do
    key_one = Key.new(@node, "hdm::integer")
    key_two = Key.new(@node, "hdm::float")
    assert_not_equal key_one, key_two
  end
  ...
  test "#hierarchies loads all hierarchies" do
    hierarchies = [Hierarchy.new(node: @node, name: "test", datadir: Dir.pwd, backend: :yaml, files: [])]
    key = Key.new(@node, "hdm::integer")
