...
class NodeTest < ActiveSupport::TestCase
  ...
  test "finds nodes with only `facts_environment` set" do
    ...
    PuppetDbClient.stub :nodes, stubbed_nodes do
      ...
      assert_includes node_names, "facts_node"
    end
  end

  test "create node object" do
    assert Node.new(hostname: 'testhost', environment: @environment)
  end

  test "two nodes are equal when their hostnames and environments are identical" do
    node_one = Node.new(hostname: "testhost", environment: @environment)
    node_two = Node.new(hostname: "testhost", environment: @environment)
    assert_equal node_one, node_two
  end

  test "two nodes are not equal when they have different hostnames" do
    node_one = Node.new(hostname: "testhost", environment: @environment)
    node_two = Node.new(hostname: "testhost2", environment: @environment)
    assert_not_equal node_one, node_two
  end

  test "two nodes are not equal when they have different environments" do
    test_environment = Environment.find("test")
    node_one = Node.new(hostname: "testhost", environment: @environment)
    node_two = Node.new(hostname: "testhost", environment: test_environment)
    assert_not_equal node_one, node_two
  end
